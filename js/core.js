...
function showRoomTypeModal() {
    if (skipRoomTypeModal) {
        skipRoomTypeModal = false; // Сбрасываем флаг
        createEmptyRoom(); // Создаем пустую комнату (как было раньше)
        return;
    }
...
function buildRectangleRoom() {
    // Получаем значения в сантиметрах
    ...
    // Конвертируем сантиметры в миллиметры (внутренняя система работает в мм)
    const lengthMm = lengthCm * 10;
    const widthMm = widthCm * 10;
    
    // Создаем прямоугольное помещение
    saveState();
    
    const centerX = 0; // Центрируем относительно начала координат
    const centerY = 0;
    ...
    // Центрируем вид на новом помещении
    setTimeout(() => {
        centerView();
    }, 100);
    ...
    // Показываем пользователю размеры в см (для подтверждения)
