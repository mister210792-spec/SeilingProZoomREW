        ...
        .then((doc) => {
            if (doc.exists) {
                ...
                if (currentUser) {
                    ...
                    // ПОСЛЕ ЗАГРУЗКИ ПЛАНА ЗАГРУЖАЕМ ЭЛЕМЕНТЫ
                ...
                db.collection('users').doc(uid).set({
                    ...
                    // После создания документа загружаем элементы (их пока нет)
                    loadCustomElementsFromFirestore(uid);
                });
            }
        })
...
function loadCustomElementsFromFirestore(uid) {
    ...
    return db.collection('users').doc(uid).collection('customElements').get()
        .then((querySnapshot) => {
            ...
            // Загружаем новые из Firestore
            querySnapshot.forEach((doc) => {
                const element = doc.data();
                const key = doc.id; // Используем ID документа как ключ
            ...
            console.log(`✅ Загружено ${querySnapshot.size} кастомных элементов из облака`);
            saveAllSettings(); // Сохраняем в localStorage как резервную копию
            initSelectors(); // Обновляем селекторы
        })
        .catch((error) => {
            console.error("❌ Ошибка загрузки кастомных элементов:", error);
        });
}
...
function completeAuth() {
    ...
    // ЗАГРУЖАЕМ КАСТОМНЫЕ ЭЛЕМЕНТЫ ИЗ FIRESTORE
    if (currentUser && currentUser.uid) {
        loadCustomElementsFromFirestore(currentUser.uid).then(() => {
            initSelectors();
        });
    } else {
        initSelectors();
    ...
    // Инициализируем мобильные обработчики после входа
    initTouchHandlers();
}
...
function savePrices() { 
    ...
    // Синхронизируем цены кастомных элементов с Firestore
    if (currentUser && currentUser.uid && db) {
        ...
        for (let key in CUSTOM_REGISTRY) {
            if (key.startsWith('custom_')) {
                // Обновляем цену в существующем документе
        ...
        batch.commit()
            .then(() => {
                console.log("✅ Цены синхронизированы с облаком");
            })
            .catch((error) => {
    ...
    if(confirm('Удалить этот элемент из списка?')) {
        ...
        // Удаляем из Firestore (если это кастомный элемент и пользователь авторизован)
        if (key.startsWith('custom_') && currentUser && currentUser.uid && db) {
            db.collection('users').doc(currentUser.uid)
              .collection('customElements').doc(key).delete()
              .then(() => {
                  console.log("✅ Элемент удален из облака:", key);
              })
              .catch((error) => {
                  console.error("❌ Ошибка удаления элемента из облака:", error);
              });
        }
...
function addNewElementConfirm() {
    ...
    const newEl = { 
        ...
        type: type, 
        shape: shape,
        createdAt: firebase.firestore.FieldValue.serverTimestamp() // для сортировки
    };
    ...
    if (currentUser && currentUser.uid && db) {
        db.collection('users').doc(currentUser.uid)
          .collection('customElements').doc(id).set(newEl)
          .then(() => {
              console.log("✅ Элемент сохранен в облако:", id);
          })
          .catch((error) => {
              console.error("❌ Ошибка сохранения элемента в облако:", error);
              alert("Элемент сохранен локально, но не синхронизирован с облаком. Проверьте соединение.");
          });
    }
...
function renderTabs() {
    ...
    rooms.forEach((r, i) => {
        ...
        t.onclick = () => { activeRoom = i; renderTabs(); draw(); }; 
        tabs.appendChild(t);
    });
